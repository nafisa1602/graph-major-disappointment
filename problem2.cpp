...
DijkstraResult dijkstraCost(int source)
{
    ...
    while(!pq.empty())
    {
        ...
        {
            if(edge.mode == BIKOLPOBUS || edge.mode == UTTARABUS || edge.mode == WALK) continue;
...
int main()
{
    ...
    {
        cout << "Cheapest cost (Car + Metro): " << result.cost[d] << " Taka\n";
        vector<int> path;
        int curr = d;
        while(curr != -1) 
        {
        ...
        cout << "\nKML file created: problem2_route.kml" << endl;
